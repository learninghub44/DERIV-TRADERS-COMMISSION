...
ALTER TABLE deriv_integrations ENABLE ROW LEVEL SECURITY;
ALTER TABLE deriv_applications ENABLE ROW LEVEL SECURITY;
ALTER TABLE markup_records ENABLE ROW LEVEL SECURITY;
...
-- Organizations: Members can view their org, owners can manage
...
CREATE POLICY "Members can view organization members" ON organization_members
  FOR SELECT USING (
    EXISTS (SELECT 1 FROM organization_members om WHERE om.organization_id = organization_members.organization_id AND om.user_id = auth.uid() AND om.status = 'active')
  );

CREATE POLICY "Org admins can manage members" ON organization_members
  FOR ALL USING (
    EXISTS (SELECT 1 FROM organization_members om WHERE om.organization_id = organization_members.organization_id AND om.user_id = auth.uid() AND om.role IN ('org_owner', 'org_admin') AND om.status = 'active')
  );

-- Deriv Integrations: Only org members can access
CREATE POLICY "Members can view organization integrations" ON deriv_integrations
  FOR SELECT USING (
    EXISTS (SELECT 1 FROM organization_members om WHERE om.organization_id = deriv_integrations.organization_id AND om.user_id = auth.uid() AND om.status = 'active')
    OR EXISTS (SELECT 1 FROM organizations o WHERE o.id = deriv_integrations.organization_id AND o.owner_id = auth.uid())
  );

CREATE POLICY "Admins can manage integrations" ON deriv_integrations
  FOR ALL USING (
    EXISTS (SELECT 1 FROM organization_members om WHERE om.organization_id = deriv_integrations.organization_id AND om.user_id = auth.uid() AND om.role IN ('org_owner', 'org_admin') AND om.status = 'active')
    OR EXISTS (SELECT 1 FROM organizations o WHERE o.id = deriv_integrations.organization_id AND o.owner_id = auth.uid())
  );

-- Deriv Applications: Same as integrations
CREATE POLICY "Members can view organization applications" ON deriv_applications
  FOR SELECT USING (
    EXISTS (SELECT 1 FROM organization_members om WHERE om.organization_id = deriv_applications.organization_id AND om.user_id = auth.uid() AND om.status = 'active')
    OR EXISTS (SELECT 1 FROM organizations o WHERE o.id = deriv_applications.organization_id AND o.owner_id = auth.uid())
  );

-- Markup Records: Same isolation
CREATE POLICY "Members can view organization markup" ON markup_records
  FOR SELECT USING (
    EXISTS (SELECT 1 FROM organization_members om WHERE om.organization_id = markup_records.organization_id AND om.user_id = auth.uid() AND om.status = 'active')
    OR EXISTS (SELECT 1 FROM organizations o WHERE o.id = markup_records.organization_id AND o.owner_id = auth.uid())
  );

CREATE POLICY "System can insert markup records" ON markup_records
  FOR INSERT WITH CHECK (true);

-- Commission Records: Same isolation
CREATE POLICY "Members can view organization commissions" ON commission_records
  FOR SELECT USING (
    EXISTS (SELECT 1 FROM organization_members om WHERE om.organization_id = commission_records.organization_id AND om.user_id = auth.uid() AND om.status = 'active')
    OR EXISTS (SELECT 1 FROM organizations o WHERE o.id = commission_records.organization_id AND o.owner_id = auth.uid())
  );

CREATE POLICY "System can insert commission records" ON commission_records
  FOR INSERT WITH CHECK (true);

-- Clients: Same isolation
CREATE POLICY "Members can view organization clients" ON clients
  FOR SELECT USING (
    EXISTS (SELECT 1 FROM organization_members om WHERE om.organization_id = clients.organization_id AND om.user_id = auth.uid() AND om.status = 'active')
    OR EXISTS (SELECT 1 FROM organizations o WHERE o.id = clients.organization_id AND o.owner_id = auth.uid())
  );

CREATE POLICY "System can insert clients" ON clients
  FOR INSERT WITH CHECK (true);

CREATE POLICY "System can update clients" ON clients
  FOR UPDATE USING (true);

-- Trading Activity: Same isolation
CREATE POLICY "Members can view organization trading" ON trading_activity
  FOR SELECT USING (
    EXISTS (SELECT 1 FROM organization_members om WHERE om.organization_id = trading_activity.organization_id AND om.user_id = auth.uid() AND om.status = 'active')
    OR EXISTS (SELECT 1 FROM organizations o WHERE o.id = trading_activity.organization_id AND o.owner_id = auth.uid())
  );

CREATE POLICY "System can insert trading activity" ON trading_activity
  FOR INSERT WITH CHECK (true);

-- Earnings: Same isolation
CREATE POLICY "Members can view organization earnings" ON earnings
  FOR SELECT USING (
    EXISTS (SELECT 1 FROM organization_members om WHERE om.organization_id = earnings.organization_id AND om.user_id = auth.uid() AND om.status = 'active')
    OR EXISTS (SELECT 1 FROM organizations o WHERE o.id = earnings.organization_id AND o.owner_id = auth.uid())
  );

CREATE POLICY "System can insert earnings" ON earnings
  FOR INSERT WITH CHECK (true);

-- Sync Jobs: Same isolation
CREATE POLICY "Members can view organization sync jobs" ON sync_jobs
  FOR SELECT USING (
    EXISTS (SELECT 1 FROM organization_members om WHERE om.organization_id = sync_jobs.organization_id AND om.user_id = auth.uid() AND om.status = 'active')
    OR EXISTS (SELECT 1 FROM organizations o WHERE o.id = sync_jobs.organization_id AND o.owner_id = auth.uid())
  );

CREATE POLICY "System can manage sync jobs" ON sync_jobs
  FOR ALL USING (true);
...
-- Audit Logs: Org admins can view, system can insert
CREATE POLICY "Admins can view organization audit logs" ON audit_logs
  FOR SELECT USING (
    EXISTS (SELECT 1 FROM organization_members om WHERE om.organization_id = audit_logs.organization_id AND om.user_id = auth.uid() AND om.role IN ('org_owner', 'org_admin') AND om.status = 'active')
    OR EXISTS (SELECT 1 FROM organizations o WHERE o.id = audit_logs.organization_id AND o.owner_id = auth.uid())
  );

CREATE POLICY "System can insert audit logs" ON audit_logs
  FOR INSERT WITH CHECK (true);

-- Subscriptions: Same as organization
CREATE POLICY "Members can view organization subscription" ON subscriptions
  FOR SELECT USING (
    EXISTS (SELECT 1 FROM organization_members om WHERE om.organization_id = subscriptions.organization_id AND om.user_id = auth.uid() AND om.status = 'active')
    OR EXISTS (SELECT 1 FROM organizations o WHERE o.id = subscriptions.organization_id AND o.owner_id = auth.uid())
  );

CREATE POLICY "System can manage subscriptions" ON subscriptions
  FOR ALL USING (true);

-- Reports: Same isolation
CREATE POLICY "Members can view organization reports" ON reports
  FOR SELECT USING (
    EXISTS (SELECT 1 FROM organization_members om WHERE om.organization_id = reports.organization_id AND om.user_id = auth.uid() AND om.status = 'active')
